alter table public.orders
  add column if not exists guest_count integer not null default 1 check (guest_count between 1 and 30);

create index if not exists orders_restaurant_guest_count_idx
  on public.orders (restaurant_id, guest_count);
...
begin
  ...
  if p_guest_count is null or p_guest_count < 1 or p_guest_count > 30 then
    raise exception 'Guest count must be between 1 and 30.';
  end if;
...
grant execute on function public.create_qr_order_transaction(text, text, text, integer, text, jsonb) to anon, authenticated;
