...
const settingsSchema = z.object({
  ...
  settings: z.object({
    ...
    bookingSlotMinutes: z.number().int().min(15).max(120),
    bookingDurationMinutes: z.number().int().min(30).max(360),
    bookingAdvanceDays: z.number().int().min(1).max(180),
    bookingMinNoticeMinutes: z.number().int().min(0).max(1440),
    bookingMaxPartySize: z.number().int().min(1).max(100),
...
export async function PATCH(request: Request) {
  ...
  const { error: settingsError } = await supabase
    ...
    .upsert({
      ...
      booking_slot_minutes: input.settings.bookingSlotMinutes,
      booking_duration_minutes: input.settings.bookingDurationMinutes,
      booking_advance_days: input.settings.bookingAdvanceDays,
