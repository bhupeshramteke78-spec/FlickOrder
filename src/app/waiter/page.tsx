...
import { WaiterKioskView } from "@/components/staff/waiter-kiosk-view";
...
export default async function WaiterPage({
  ...
  const staffSession = await getStaffSessionFromCookie("waiter");

  if (staffSession && (!slug || staffSession.restaurantSlug === slug)) {
    const orders = await getWaiterOrders(staffSession.restaurantId);
    return (
      <WaiterKioskView
        orders={orders}
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#090D10] p-4 text-center text-white">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-400">Please scan the Waiter QR code provided in your restaurant dashboard.</p>
        </div>
      </div>
    );
  }
...
async function getWaiterOrders(restaurantId: string) {
